...
describe('사전 키가 화면에서 실제로 읽힌다', () => {
  ...
  const INTENTIONALLY_UNUSED = new Set([
    ...
    // ── 지역화된 404: 화면은 만들었지만 **경계가 발동하지 않아 되돌렸다** ──
    //
    // `src/app` 에 not-found.tsx 가 없어서 숨겨진 질문 링크를 받은 베트남어
    // 사용자가 스타일도 헤더도 없는 영어 404 를 만난다. 그래서 `[locale]/
    // not-found.tsx` 에 지역화 화면을 만들었는데, **Next 16 에서 그 중첩 경계가
    // 쓰이지 않았다** — 운영과 같은 standalone 서버로 확인했고
    // `/ko/visa/zzz` 가 여전히 Next 기본 404 를 렌더했다.
    //
    // 발동하지 않는 경계를 두는 것은 없는 것보다 나쁘다(있다고 믿게 만든다).
    // 그래서 되돌렸다. 다시 할 때 확인할 것: 루트 `app/not-found.tsx` +
    // `usePathname()` 로 로케일을 읽는 방식(`lib/i18n/t` 는 provider 없이 동작한다).
    'question.notFound',
    'question.backToList',
