...
| `npm run check:seo` | 상세 페이지가 **색인 가능한 상태로 서버 렌더**되는지 (dev 서버 + BE 필요) |

> 세 검사는 `scripts/lib/check-report.mjs` 로 **보고 형식과 종료코드를 공유**한다.
> 검사가 더 늘어날 예정이라(sitemap·hreflang) 각자 다시 구현하지 않는다.
