...
| `/[locale]/regions/[region]` | 지역 허브 — `ansan` `hwaseong` `seoul` … | 1 |
| `/[locale]/nationality/[code]` | 국적 허브 — 아포스티유·본국 서류처럼 **답이 국적별인 것만** | 2 |
| `/[locale]/guides` · `/guides/[slug]` | 가이드 (반복 질문이 승격된 문서) | 2 |
| `/[locale]/checklists/[slug]` | 연차별 체크리스트 | 2 |
| `/[locale]/users/[handle]` | 공개 프로필 — **답변자 신뢰의 근거** | 1 |
...
| **2** | 가이드·체크리스트·알림·국적 허브 | 콘텐츠가 쌓인 뒤 |
...
**색인 표면은 12종뿐이고, 그중 실제 트래픽은 `questions/[id]/[slug]` 와 허브 3종에 몰린다.**
나머지는 그 둘을 받치는 구조다.
