...
**이제 CI 가 두 번 다 돌린다**(`ci.yml` 의 backend job 이 Postgres 서비스 컨테이너를
띄운다). 그전까지는 이 절이 계약이라고 선언해 놓고 **아무도 두 번째를 돌리지 않았다** —
개발 기계에 Postgres 나 docker 가 없으면 사람도 못 돌리므로, 사실상 어디서도
검증되지 않는 상태였다. *사람이 기억해야만 지켜지는 규칙은 언젠가 깨진다.*

두 겹의 가드가 붙어 있다:

- `loadAppConfig` 가 **모르는 `DB_DRIVER` 값에 던진다.** 예전에는 검사 없는
  캐스트라 `Prisma`·`postgres`·빈 문자열이 조용히 인메모리로 떨어졌다 —
  운영에서 그 뜻은 <앱은 정상으로 뜨고 헬스체크도 통과하는데 재시작마다
  모든 글이 사라진다> 였다(`app.config.spec.ts`)
- e2e 가 **DI 가 실제로 묶은 저장소 클래스**를 확인한다(`assertDriverBound`).
  설정값을 환경변수와 비교하는 것은 동어반복이라, 두 번째 실행에서 `DB_DRIVER` 를
  빠뜨리면 <prisma 드라이버> 라는 이름의 스텝이 인메모리를 검사하며 초록이 된다
